...
var fileLoader = function fileLoader(folderPath) {
  ...
  var dir = folderPath;
  var schemafiles = recursive === true ? recursiveReadDirSync(dir) : readDirSync(dir);
...
var _makeRestDefinitions = function _makeRestDefinitions(defs) {
  return defs.filter(function (def) {
    return _isNonMergeableTypeDefinition(def) && !isObjectSchemaDefinition(def);
  }).map(function (def) {
...
var mergeResolvers = function mergeResolvers(resolvers) {
  return resolvers.length === 1 ? resolvers[0] : index.all(resolvers);
};
